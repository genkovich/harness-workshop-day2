...
- `src/chat.ts`: місток між чатом і Flue. Отримує id чату й текст, повертає відповідь агента. У ньому вся суть етапу.
- `src/bot.ts`: обвʼязка Telegram на бібліотеці grammY. Вона майже однакова для будь-якого бота, тож збираємо її швидко, по кілька рядків.
...
Створи файл. Будуємо його зверху вниз, кожен фрагмент додаємо в кінець файла.
...
`Bot` з grammY отримує повідомлення з Telegram. `start` і `sqlite` запускають Flue у нашому процесі. `askAgent` це наш місток з попереднього кроку.

Без токена бот не запуститься, тож перевіряємо його першим:
...
`flue run` запускав Flue сам. Бот це наш власний процес, тож Flue стартуємо явно і кажемо, яких агентів він обслуговує. `sqlite('./bot.db')` задає, де зберігати історію розмов. Без цього параметра історія жила б лише в памʼяті і зникала б при перезапуску.
...
Головний обробник. Кожне текстове повідомлення йде в `askAgent` разом з id свого чату:
...
Id бере код з даних Telegram, модель його не вибирає. Відповідь надсилаємо шматками по 4000 символів, бо довше повідомлення Telegram не прийме. Якщо агент упав, причину друкуємо в терміналі, а користувачу коротко пишемо, що не вийшло.

Помилки самого Telegram, наприклад обрив мережі, ловимо окремо, щоб бот не падав:
...
`bot.stop()` встигає сказати Telegram, які повідомлення вже оброблено. Без цього після перезапуску Telegram надішле останнє повідомлення ще раз, і агент, наприклад, збереже ту саму нотатку двічі. А перезапускати бота ти будеш після кожної зміни коду.
...
`bot.start()` починає питати Telegram про нові повідомлення (long polling) і працює, доки процес не зупинять.
